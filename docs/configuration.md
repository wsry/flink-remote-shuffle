...
| `remote-shuffle.job.compression.codec` | String | `LZ4` | 1.1.0 | false | "The codec to be used when compressing shuffle data, only `LZ4`, `LZO` and `ZSTD` are supported now. Through tpc-ds test of these three algorithms, the results show that `LZ4` algorithm has the highest compression and decompression speed, but the compression ratio is the lowest. `ZSTD` has the highest compression ratio, but the compression and decompression speed is the slowest, and LZO is between the two." |
